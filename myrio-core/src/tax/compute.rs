...
pub struct TaxTreeCompute {
    ...
    pub(crate) kmer_counts_repr: SFVec,
...
impl TaxTreeCompute {
    pub fn get_kmer_counts_repr(&self) -> SFVec {
        self.kmer_counts_repr.clone()
    }
    ...
    pub fn from_store_tree(
        ...
        let mut kmer_counts_repr =
            if let Some(sfvec_idx) = store.pre_computed.iter().copied().position(|k| k_cluster == k) {
                (0..repr_samples)
                    ...
                    .fold(SFVec::new(0), |acc, x| acc.merge_with_and_apply(x, |x, y| x + y))
            } else {
                (0..repr_samples)
                    .map(|_| {
        ...
        kmer_counts_repr /= kmer_counts_repr.sum();
            ...
            .map_or_else(
                || match cache_opt {
                    CacheOptions::Enabled { zstd_compression_level, zstd_multithreading_opt } => {
                        store.compute_and_append_kmer_counts(k_search, max_consecutive_N_before_gap);
                        store.encode_to_file(zstd_compression_level, zstd_multithreading_opt)?;
                        Ok::<usize, Error>(store.pre_computed.len())
                    },
                    CacheOptions::Disabled => {
                        store.compute_and_overwrite_kmer_counts(k_search, max_consecutive_N_before_gap);
